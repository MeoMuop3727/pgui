...
class StyleAlert:
    ...
    - Title frame  : top portion, height = size[1] * per_height_title_frame
    - Content frame: remaining portion, contains the wrapped text and close button
    ...
    bg_title_color : ColorType, optional
        Custom title bar background color. Overrides the type-based color if set.
    ...
    per_height_title_frame : float
        Fraction of total height allocated to the title bar. Defaults to 0.20.
    visible : bool
        Whether the alert is rendered. Defaults to True.
...
class Alert:
    ...
    ----------------------
    1. Border         (slightly larger rect behind the alert)
    2. Title frame    (colored bar with centered title text and close button)
    3. Content frame  (TextBox with word-wrapped content text)
